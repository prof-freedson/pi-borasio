...
export default function OferecerCaronaPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Formata a data e hora para ISO string
      // Assumindo fuso horário local, cria objeto Date
      const dataHoraString = `${formData.data}T${formData.horario}:00`;
      const dataHora = new Date(dataHoraString);

      const payload = {
        motoristaId: 1, // Temporário: substituir pelo ID do usuário logado quando houver auth
        tipo: tipoCarona,
      ...
      if (!response.ok) {
        throw new Error('Falha ao criar oferta de carona');
      }

      // Opcional: manter o localStorage como cache ou histórico local
      const novaCorrida = await response.json();
      ...
      <div className="max-w-3xl mx-auto px-6 pt-12 pb-24 relative z-10">
        ...
        <div className="mb-12 grid grid-cols-2 gap-4 animate-in fade-in slide-in-from-top-4 duration-700">
          <button
            ...
            className={`relative group p-6 rounded-[32px] border-2 transition-all duration-300 ${tipoCarona === 'geral'
          ...
          <button
            ...
            className={`relative group p-6 rounded-[32px] border-2 transition-all duration-300 ${tipoCarona === 'grupo'
            ...
            <div className="flex items-center justify-between relative mb-8">
              ...
              {[1, 2, 3].map((stepNumber) => (
                <div key={stepNumber} className="relative z-10 flex flex-col items-center">
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center font-black text-lg transition-all duration-500 ${step >= stepNumber
                  ...
                  <div className={`absolute -bottom-10 whitespace-nowrap text-[10px] font-black uppercase tracking-widest transition-all duration-500 ${step >= stepNumber ? 'text-[#004d2b]' : 'text-gray-300'
                    }`}>
                  ...
                  <div className="flex bg-gray-50/50 p-2 rounded-3xl border border-gray-100 shadow-inner overflow-x-auto gap-2">
                    {[1, 2, 3, 4, 5, 6].map(num => (
                      <button
                        ...
                        className={`flex-1 min-w-[60px] py-4 rounded-2xl font-black transition-all ${formData.assentos === num
                  ...
                  <div>
                    ...
                    <div className="flex bg-gray-100/50 p-1.5 rounded-[22px] border border-gray-100">
                      <button
                        ...
                        className={`flex-1 py-3 px-4 rounded-2xl font-bold text-sm transition-all ${formData.arCondicionado
